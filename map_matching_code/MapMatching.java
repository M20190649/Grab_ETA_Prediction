...
public class MapMatching {
    ...
    public HashMap<Integer, ArrayList<Pair<Double, Date>>> edgeIDsToSpeedTime;
    ...
    public MapMatching(GraphHopper graphHopper, HintsMap hints) {
        ...
        this.edgeIDsToSpeedTime = new HashMap<Integer, ArrayList<Pair<Double, Date>>>();
...
//                            from, to, penalizedPathDistance);
    ...
    private List<EdgeMatch> computeEdgeMatches(List<SequenceState<State, Observation, Path>> seq, Map<String, EdgeIteratorState> virtualEdgesMap) {
        ...
        for (SequenceState<State, Observation, Path> transitionAndState : seq) {
            Double speed = transitionAndState.state.getEntry().getPoint().speed;
            Date measureTime = transitionAndState.state.getEntry().getPoint().time;
            ...
            if (transitionAndState.transitionDescriptor != null) {
                for (EdgeIteratorState edge : transitionAndState.transitionDescriptor.calcEdges()) {
                    ...
                    Integer edgeId = newDirectedRealEdge.getEdge();
                    if (newDirectedRealEdge instanceof VirtualEdgeIteratorState) {
                        // first, via and last edges can be virtual
                        VirtualEdgeIteratorState vEdge = (VirtualEdgeIteratorState) newDirectedRealEdge;
                        edgeId = vEdge.getOriginalEdgeKey() / 2;
                    }
                    if (edgeIDsToSpeedTime.containsKey(edgeId) == false) {
                        edgeIDsToSpeedTime.put(edgeId, new ArrayList<Pair<Double, Date>>());
                    }
                    edgeIDsToSpeedTime.get(edgeId).add(new Pair<Double, Date>(speed, measureTime));
